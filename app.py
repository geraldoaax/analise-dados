...
def get_processed_production_by_frota_carga_nova(data_inicio=None, data_fim=None, tipos_input=None, frota_transporte=None):
    """Obtém dados de produção (soma de massa) por frota de carga (Frota carga) com filtros avançados"""
    logger.info("🔄 Processando dados de produção por frota de carga (Frota carga) com filtros avançados...")
    process_start = time.time()
    ...
    # Filtrar frotas de carga válidas (não vazias e diferentes de "-")
    df = df[df['Frota carga'] != '-']
    df = df[df['Frota carga'].str.strip() != '']
    ...
    logger.info("📊 Agrupando dados por frota de carga (Frota carga) e somando massa...")
    # Primeiro, calcular totais por frota de carga para identificar os maiores
    totals_by_frota_carga = df.groupby('Frota carga')['Massa'].sum().sort_values(ascending=False)
    logger.info(f"📋 Frotas de carga (Frota carga) encontradas: {len(totals_by_frota_carga)}")
    ...
    # Criar nova coluna agrupando frotas de carga menores em "Outros"
    df['Frota_Carga_Agrupada'] = df['Frota carga'].apply(
        lambda x: x if x in top_frotas_carga else 'Outros'
    )
    ...
    # Renomear coluna para manter compatibilidade com frontend
    production_data = production_data.rename(columns={'Frota_Carga_Agrupada': 'Frota carga'})
    
    # Converter período para string e ordenar
    production_data['AnoMes'] = production_data['AnoMes'].astype(str)
    production_data = production_data.sort_values(['AnoMes', 'Frota carga'])
...
@app.route('/api/production_by_frota_carga_nova')
def production_by_frota_carga_nova():
    logger.info("🚀 API production_by_frota_carga_nova chamada")
    api_start_time = time.time()
    ...
    try:
        ...
        # Usar função com filtros de data, tipos de input e frota de transporte
        result = get_processed_production_by_frota_carga_nova(data_inicio, data_fim, tipos_input, frota_transporte)
        
        total_api_time = time.time() - api_start_time
        logger.info(f"✅ API production_by_frota_carga_nova concluída com sucesso!")
        logger.info(f"⏱️  Tempo total da API: {total_api_time:.2f}s")
        logger.info(f"📊 Dados retornados: {len(result)} registros")
        ...
        if result:
            logger.info("📋 Primeiros registros:")
            for i, record in enumerate(result[:5]):
                logger.info(f"   {i+1}. {record['AnoMes']} - {record['Frota carga']}: {record['massa_total']:,.2f} kg")
            if len(result) > 5:
                logger.info(f"   ... e mais {len(result) - 5} registros")
        ...
        error_time = time.time() - api_start_time
        logger.error(f"❌ Erro na API production_by_frota_carga_nova após {error_time:.2f}s: {str(e)}")
        logger.exception("Detalhes do erro:")
        return jsonify({'error': f'Erro ao processar dados: {str(e)}'})
